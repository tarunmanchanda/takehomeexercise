...
- **`TestEntityManager` lives in yet another package**, `org.springframework.boot.jpa.test.autoconfigure` — not `org.springframework.boot.data.jpa.test.autoconfigure` where `@DataJpaTest` itself lives, even though both ship in the same `spring-boot-starter-data-jpa-test` dependency. Confirmed via `unzip -l` on `spring-boot-jpa-test-4.1.0.jar` (a further sub-module of `spring-boot-data-jpa-test`) after a plain package-name guess failed to resolve. Needed for tests that must force two independent, non-cached loads of the same row (e.g. proving optimistic locking actually rejects a stale concurrent write) via `persistFlushFind`/`detach`/`clear`.
