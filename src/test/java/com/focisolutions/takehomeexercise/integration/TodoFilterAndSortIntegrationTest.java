...
class TodoFilterAndSortIntegrationTest {
    ...
    void givenTodosWithVariedStateAndDueDates_whenFilteringAndSorting_thenResultsMatchEachCriteriaTest() throws Exception {
        ...
        mockMvc.perform(authorized(post("/api/v1/todos")).contentType("application/json")
                .content("{\"title\":\"Charlie\"}")).andExpect(status().isCreated());
        final String alphaLocation = mockMvc.perform(authorized(post("/api/v1/todos")).contentType("application/json")
                        .content("{\"title\":\"Alpha\",\"dueDate\":\"" + overdueDueDate + "\"}"))
                .andExpect(status().isCreated()).andReturn().getResponse().getHeader("Location");
        final String bravoLocation = mockMvc.perform(authorized(post("/api/v1/todos")).contentType("application/json")
                        .content("{\"title\":\"Bravo\"}"))
                .andExpect(status().isCreated()).andReturn().getResponse().getHeader("Location");
        ...
        mockMvc.perform(authorized(get("/api/v1/todos")).param("status", "OVERDUE"))
        ...
        // then: only Bravo
        mockMvc.perform(authorized(get("/api/v1/todos")).param("status", "COMPLETED"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.length()").value(1))
        ...
        mockMvc.perform(authorized(get("/api/v1/todos")).param("status", "INCOMPLETE"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.length()").value(2));

        // when: sort ALL by title ascending
        // then: Alpha, Bravo, Charlie in order
        mockMvc.perform(authorized(get("/api/v1/todos")).param("sortBy", "TITLE").param("direction", "ASC"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$[0].title").value("Alpha"))
